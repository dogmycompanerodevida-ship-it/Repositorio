...
function dibujarPinMapa(ctx, x, y, color) {
    const radio = 11;
    const altoPin = 30;
    ctx.save();
...
// GALERIA DE RECORRIDOS (capturas guardadas al finalizar cada paseo) --
// disponible tanto en el panel de administrador como en el del paseador,
// con el mismo formato que la Galeria de Fotos. Se define y se llama
// aqui, a nivel superior del archivo, para que corra sin importar cual
// de las dos paginas (admin.html o paseador.html) sea la que cargo --
// si el boton correspondiente no existe en esa pagina, no hace nada.
// Abre cualquier imagen (miniatura de recorrido, foto, etc.) en pantalla
// completa, como un visor normal -- se cierra tocando la imagen o la X.
window.abrirImagenCompleta = function(src) {
    ...
    capa.onclick = () => document.body.removeChild(capa);
...
function montarGaleriaRecorridos(idBoton, idBox) {
    ...
    btn.addEventListener("click", async function() {
        ...
        try {
            const [statsSnap, perrosSnap] = await Promise.all([
                get(ref(db, "estadisticas")),
                get(ref(db, "perros"))
            ]);
            ...
            const lista = [];
            Object.entries(stats).forEach(([perroId, s]) => {
                if (!s.historial) return;
                Object.values(s.historial).forEach(h => {
                    if (!h.imagenRuta) return;
                    lista.push({
                        fecha: h.fecha,
                        metros: h.metros || 0,
                        minutos: h.minutos || 0,
                        imagenRuta: h.imagenRuta,
                        nombrePerro: (perros[perroId] && perros[perroId].nombre) || "Perro"
                    });
                });
            });
            ...
            lista.forEach(r => {
                ...
                html += `
                    <div style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                        <img src="${r.imagenRuta}" onclick="abrirImagenCompleta(this.src)" style="width:100%;height:120px;object-fit:cover;cursor:pointer;">
                        <p style="font-size:11px;padding:8px;margin:0;color:#666;">
                            🐶 ${r.nombrePerro}<br>
                            📏 ${(r.metros/1000).toFixed(2)}km en ${r.minutos}min<br>
                            🕒 ${fecha}
                        </p>
                    </div>
                `;
            ...
            console.error("Error cargando galeria de recorridos:", err);
            box.innerHTML = '<p style="text-align:center;padding:20px;color:#c62828;">No se pudo cargar (revisa tu conexión o los permisos de la base de datos).</p>';
        }
    });
}
montarGaleriaRecorridos("btnGaleriaRecorridos", "galeriaRecorridosBox");
...
if (nombrePaseadorEl) {
    ...
    // Pone la banderita roja en la ultima posicion conocida, justo cuando
    // el paseador toca "Finalizar". El GPS se queda prendido 30 segundos
    // mas despues de esto (ver finalizarPaseoCompleto), asi que el punto
    // final real de la ruta puede quedar un poco mas adelante -- esta
    // bandera marca el momento exacto en que se toco el boton.
    function marcarBanderaFinal() {
        if (!mapaPropio || !markerPropioActual) return;
        if (markerFinPropio) { mapaPropio.removeLayer(markerFinPropio); markerFinPropio = null; }
        const posActual = markerPropioActual.getLatLng();
        const iconoFin = crearIconoBandera("#c62828");
        markerFinPropio = L.marker([posActual.lat, posActual.lng], { icon: iconoFin })
            .addTo(mapaPropio).bindPopup("🚩 Aquí finalizaste el paseo").openPopup();
    }
    ...
    onValue(ref(db, "usuarios/" + uid), snapshot => {
        ...
        if (data.activo && data.paseoActualId) {
            ...
            paseoEnCurso = true;
            requestWakeLock();
            // Antes esto se ejecutaba en CADA actualizacion de ubicacion
            // (cada pocos segundos), reiniciando el GPS nativo sin parar
            // durante todo el paseo -- eso era lo que rompia la espera de
            // 30 segundos y la captura al finalizar. Ahora solo arranca
            // el GPS una vez, al inicio real del paseo.
    ...
    let markersEntregados = [];

    // Pone una bandera roja en el punto donde se entrega a un perro
    // especifico (cuando van varios en el mismo paseo). A diferencia de
    // marcarBanderaFinal, esta NO borra las banderas anteriores -- asi se
    // ven todas las entregas del recorrido, una por cada perro.
    function marcarBanderaEntrega(nombrePerro) {
        if (!mapaPropio || !markerPropioActual) return;
        const pos = markerPropioActual.getLatLng();
        const icono = crearIconoBandera("#c62828");
        const marker = L.marker([pos.lat, pos.lng], { icon: icono })
            .addTo(mapaPropio)
            .bindPopup("🚩 Aquí se entregó a " + nombrePerro);
        markersEntregados.push(marker);
    }

    // Guarda las estadisticas de UN perro en el momento en que se entrega,
    // sin tocar a los demas que siguen en el paseo. Usa el recorrido
    // acumulado hasta ahora (paseo.ubicaciones) y el "inicio" propio de
    // ese perro (por si se sumo al paseo mas tarde que los demas).
    async function guardarEntregaIndividual(paseoId, paseo, perroId, perro, inicioPerroISO) {
        const ahora = new Date();
        const inicioPerro = new Date(inicioPerroISO || paseo.inicio);
        const minutos = Math.max(1, Math.round((ahora - inicioPerro) / 60000));
        ...
        try {
            ...
            stats.ultimoPaseo = ahora.toISOString();
            stats.historial = stats.historial || {};
            stats.historial[fechaKey] = { minutos, metros: distancia, fecha: ahora.toISOString(), paseoId, imagenRuta };
            await update(statsRef, stats);
        } catch (e) {
            console.error("Error guardando estadisticas individuales de " + perroId + ":", e);
        }
    }
    ...
    window.togglePerroEnPaseo = async function(perroId) {
        ...
        if (!data.activo) {
            ...
            if (perrosEnPaseo[perroId]) {
                const inicioEstePerro = perrosEnPaseo[perroId].inicio;
                delete perrosEnPaseo[perroId];

                // Bandera + estadisticas propias de ESTE perro, en el
                // punto exacto donde se le entrega -- sin afectar a los
                // demas que siguen en el paseo.
                marcarBanderaEntrega(perro.nombre);
                await guardarEntregaIndividual(paseoId, paseo, perroId, perro, inicioEstePerro);
                ...
                if (Object.keys(perrosEnPaseo).length === 0) {
                    ...
                    alert(perro.nombre + " entregado. Ya se entregaron todos, paseo finalizado.");
                } else {
                    await update(ref(db, "paseos/" + paseoId), { perros: perrosEnPaseo });
                    alert(perro.nombre + " entregado. Se guardó su recorrido; el GPS sigue activo para los demás.");
                }
            } else {
    ...
    window.finalizarPaseoCompleto = async function() {
        try {
            ...
            // 1) Bandera roja en el punto donde se toco "Finalizar".
            marcarBanderaFinal();

            // 2) Dejamos el GPS 30 segundos mas prendido (no se toca
            // "activo" todavia, asi que el watcher sigue registrando
            // puntos) para que la ruta guardada incluya el tramo final
            // completo antes de generar la captura.
            if (btnFinalizar) {
                btnFinalizar.disabled = true;
                btnFinalizar.textContent = "⏳ Guardando ultimo tramo (5s)...";
            }
            ...
            await update(ref(db, "usuarios/" + uid), { activo: false, paseoActualId: null, lat: null, lon: null });
            alert("Paseo finalizado completamente. La captura del recorrido ya quedo guardada.");
        } catch (err) {
            ...
            if (btnFinalizar) {
                ...
                btnFinalizar.textContent = "⏹ Finalizar Paseo Completo";
            }
        }
    };
...
if (listaPaseadoresEl) {
    ...
    onValue(paseadoresQuery, snapshot => {
        ...
        Object.entries(paseadores).forEach(([uid, p]) => {
            ...
            get(perrosQuery).then(async perrosSnap => {
                const perros = perrosSnap.val() || {};
                const count = Object.keys(perros).length;
                const div = document.createElement("div");
                ...
                // Si esta en paseo, se muestra un boton para entregar a
                // cada perro por separado (igual que en el panel del
                // paseador), en vez de un solo boton que cierre a todos
                // de golpe.
                let entregasHtml = "";
                if (p.activo && p.paseoActualId) {
                    const paseoSnap = await get(ref(db, "paseos/" + p.paseoActualId + "/perros"));
                    const perrosEnEstePaseo = paseoSnap.val() || {};
                    const filas = Object.entries(perrosEnEstePaseo).map(([perroId, datosPerro]) => `
                        <div style="display:flex;justify-content:space-between;align-items:center;background:#fff8e1;border-radius:8px;padding:6px 10px;margin:4px 0;">
                            <span style="font-size:13px;">🐕 ${datosPerro.nombre}</span>
                            <button onclick="entregarPerroDesdeAdmin('${uid}','${perroId}')" class="btn-morado" style="margin:0;padding:4px 10px;font-size:12px;">🚩 Entregar</button>
                        </div>
                    `).join("");
                    entregasHtml = `<div style="margin:8px 0;">${filas || '<p style="font-size:12px;color:#888;">Sin perros en el paseo.</p>'}</div>`;
                }
                ...
                div.innerHTML = `
                    ...
                    ${p.activo ? `<button onclick="finalizarPaseoDesdeAdmin('${uid}')" class="btn-morado">⏹ Finalizar Paseo (todos de golpe)</button>` : ''}
                `;
...
window.entregarPerroDesdeAdmin = async function(paseadorId, perroId) {
    try {
        const userSnap = await get(ref(db, "usuarios/" + paseadorId));
        const data = userSnap.val();
        if (!data || !data.activo || !data.paseoActualId) {
            alert("Este paseador ya no tiene paseo activo.");
            return;
        }
        const paseoId = data.paseoActualId;
        const paseoSnap = await get(ref(db, "paseos/" + paseoId));
        const paseo = paseoSnap.val();
        if (!paseo) { alert("No se encontro el paseo."); return; }
        const perrosEnPaseo = paseo.perros || {};
        if (!perrosEnPaseo[perroId]) { alert("Ese perro ya no esta en este paseo."); return; }
        const perroSnap = await get(ref(db, "perros/" + perroId));
        const perro = perroSnap.val();
        if (!perro) { alert("No se encontro el perro."); return; }

        if (!confirm("¿Marcar a " + perro.nombre + " como entregado?")) return;

        const inicioEstePerro = perrosEnPaseo[perroId].inicio;
        delete perrosEnPaseo[perroId];

        // Mismas estadisticas y logica que la entrega individual del
        // paseador -- solo que aqui no hay mapa propio para poner la
        // bandera (el admin no tiene ese mapa personal), asi que solo se
        // guardan las estadisticas y la captura del perro.
        const ahora = new Date();
        const inicioPerro = new Date(inicioEstePerro || paseo.inicio);
        const minutos = Math.max(1, Math.round((ahora - inicioPerro) / 60000));
        const distancia = calcularDistanciaRuta(paseo.ubicaciones);
        const fechaKey = ahora.toISOString().split("T")[0];
        const imagenRuta = generarImagenRuta(paseo.ubicaciones);
        ...
        if (Object.keys(perrosEnPaseo).length === 0) {
            await update(ref(db, "paseos/" + paseoId), { perros: perrosEnPaseo, fin: ahora.toISOString(), estado: "completado", distancia });
            await update(ref(db, "usuarios/" + paseadorId), { activo: false, paseoActualId: null, lat: null, lon: null });
            alert(perro.nombre + " entregado. Era el ultimo, paseo finalizado.");
        } else {
            await update(ref(db, "paseos/" + paseoId), { perros: perrosEnPaseo });
            alert(perro.nombre + " entregado. Los demas siguen en el paseo.");
        }
    } catch (err) {
        console.error("Error entregando perro desde admin:", err);
